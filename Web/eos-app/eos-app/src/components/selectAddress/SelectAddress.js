...
const SelectAddress = ({
  ...
  return (
    <div
      className={
        hiddenTitle
          ? "w-full my-3 mb-4 md:w-full md:mx-0 lg:w-[20%] lg:mx-0"
          : "w-full mb-4"
      }
    >
      {hiddenTitle ? null : (
      ...
      >
        <option className="pl-4 pr-10 py-3 shadow" value="None">
          {hiddenTitle ? label : `--Chọn: ${label}--`}
        </option>
        {options?.map((item) => {
          return (
            <option key={address(type, item).id} value={address(type, item).id}>
